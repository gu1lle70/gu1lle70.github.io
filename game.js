...
function game(data) {
  ...
  terminalOut(
    "<p><strong>¡Bienvenidos a ENTIerrame!</strong> El juego de terror definitivo.</p>"
  );

  terminalOut(
    "<p>Te encuentras en " +
      game_data.rooms[current_room].name +
      ". ¿Qué quieres hacer?</p>"
  );
}
...
function parseCommand(command) {
  switch (command) {
    case "ver":
      terminalOut(
        "<p>" + game_data.rooms[current_room].description + "</p>"
      );
      break;
    ...
    case "coger":
      ...
      terminalOut(
        "<p>Los items que hay en la sala son: " + items + "</p>"
      );
    ...
    case "inventario":
      ...
      terminalOut(
        "<p>Este es tu inventario: " + items_inventory + "</p>"
      );
    ...
    default:
      terminalOut(
        "<p><strong>ERROR:</strong> Comando <strong>" +
          command +
...
function parseInstruction(instruction) {
  switch (instruction[0]) {
    case "ver":
      ...
      if (item_number < 0) {
        terminalOut(
          "<p>El objeto<strong> " +
            instruction[1] +
            "</strong> no está en la habitación</p>"
        );
        return;
      }
    ...
    case "ir":
      ...
      if (door_number < 0) {
        terminalOut("Puerta incorrecta, intenta otra vez");
        return;
      }

      let room_number = findRoomNumber(
        game_data.doors[door_number].rooms[0]
      );

      if (room_number == current_room) {
        current_room = findRoomNumber(
          game_data.doors[door_number].rooms[1]
        );
      } else {
      ...
      terminalOut(
        "<p>Has cambiado de habitación a  " +
          next_room_name +
          "</p>"
      );
    ...
    case "coger":
      game_data.rooms[current_room].items.forEach(function (item) {
        if (item == instruction[1]) {
          let item_num = game_data.rooms[current_room].items.indexOf(
            item
          );
          ...
          game_data.rooms[current_room].items.forEach((item) => {
            if (item == instruction[1]) {
              items_picked.push(
                game_data.rooms[current_room].items.splice(
                  item_num,
                  1
                )
              );
          ...
          terminalOut(
            "<p>El objeto<strong> " +
              item +
              "</strong> se ha añadido a tu inventario</p>"
          );
          return;
        }
      });
    ...
    case "inventario":
      ...
      if (item_inventory_num < 0) {
        terminalOut(
          "<p>El objeto<strong> " +
            instruction[1] +
            "</strong> no está en tu inventario</p>"
        );
        return;
      }

      let item_inventory_description =
        game_data.items[item_inventory_num].description;
...
function executeCommand() {
  ...
  if (command.length == 0 || command == "") {
    terminalOut(
      "<p><strong>ERROR:</strong> Escribe lo que quieres hacer</p>"
    );
    return;
  }
...
fetch("https://gu1lle70.github.io/game.json")
